...
async function seedTariffeLombardia2026() {
  ...
  const config = await prisma.configurazioneBollo.upsert({
    where: {
  ...
  // Elimina tariffe esistenti per questa configurazione
  await prisma.tariffaBollo.deleteMany({
    where: { idConfigurazione: config.id },
  });
  ...
  const tariffeMassaRimorchiabile = [
    { tariffa: 1, descrizioneRange: 'massa complessiva inferiore a 18 ton', annuale: 267.00, quadrimestrale: 89.00 },
    { tariffa: 2, descrizioneRange: 'massa complessiva da 18 ton in su', annuale: 585.00, quadrimestrale: 195.00 },
    { tariffa: 3, descrizioneRange: '2 assi', annuale: 585.00, quadrimestrale: 195.00 },
    { tariffa: 3, descrizioneRange: '3 assi', annuale: 825.00, quadrimestrale: 275.00 },
  ];
  ...
  // ESENZIONI
  // =====================================================
  // Elimina esenzioni esistenti
  await prisma.$executeRaw`DELETE FROM "esenzioni_bollo" WHERE "id_configurazione" = ${config.id}`;

  // Veicoli elettrici - esenzione totale per 5 anni, poi riduzione 75%
  await prisma.$executeRaw`
    INSERT INTO "esenzioni_bollo"
    ("id_configurazione", "tipo_esenzione", "percentuale_riduzione", "alimentazione", "descrizione", "note", "createdAt", "updatedAt")
    VALUES
    (${config.id}, 'TOTALE', NULL, 'Elettrico', 'Veicoli elettrici - esenzione totale per i primi 5 anni', 'Dalla prima immatricolazione', NOW(), NOW())
  `;

  // Veicoli GPL/Metano - possibile riduzione regionale
  await prisma.$executeRaw`
    INSERT INTO "esenzioni_bollo"
    ("id_configurazione", "tipo_esenzione", "percentuale_riduzione", "alimentazione", "descrizione", "note", "createdAt", "updatedAt")
    VALUES
    (${config.id}, 'PARZIALE', 25, 'GPL', 'Veicoli GPL - riduzione 25%', 'Verifica normativa regionale vigente', NOW(), NOW())
  `;
  ...
  await prisma.$executeRaw`
    ...
    ("id_configurazione", "tipo_esenzione", "percentuale_riduzione", "alimentazione", "descrizione", "note", "createdAt", "updatedAt")
    VALUES
    (${config.id}, 'PARZIALE', 25, 'Metano', 'Veicoli Metano - riduzione 25%', 'Verifica normativa regionale vigente', NOW(), NOW())
  `;

  // Veicoli ultratrentennali non iscritti a registri
  await prisma.$executeRaw`
    INSERT INTO "esenzioni_bollo"
    ("id_configurazione", "tipo_esenzione", "percentuale_riduzione", "anni_da_immatricolazione", "descrizione", "note", "createdAt", "updatedAt")
    VALUES
    (${config.id}, 'PARZIALE', 50, 30, 'Veicoli ultratrentennali non iscritti a registri storici - riduzione 50%', 'Solo se non circolanti su strada', NOW(), NOW())
  `;

  console.log('Tariffe Lombardia 2026 inserite con successo!');
}

// Esporta la funzione per poterla chiamare dal seed principale
export { seedTariffeLombardia2026 };

// Se eseguito direttamente
if (require.main === module) {
  seedTariffeLombardia2026()
    .catch((e) => {
      console.error(e);
      process.exit(1);
    })
